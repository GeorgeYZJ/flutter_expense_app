...
class GA(object):
     def __init__(self, gene_size, gene_range, crossoverR, mutationR, popSize):
         ...
         self.pop = np.random.choice(gene_size, size=(popSize, gene_size)).astype(np.int8) #create the poplation accroding to the population size and the target sentence length.
    ...
    def crossover(self,parent,pop):  # pick a random entity's gene in the pop and mate it with the parent's gene provided
        if np.random.rand() < self.crossoverR:
            ...
            parent[cross_points] = pop[ram_entity, mate_points] # mate the provided parents with the chosen one on the mate points of the gene.
